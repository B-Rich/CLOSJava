 ...
 * @author Threadcount
 */
public class Sector_1 extends GameMode {
    ...
    public Sector_1(ModeManager MM) {
        super(MM);
        init();
    }
    ...
    public void init() {
        ...
        this.sky = new Bckgrnd("/Background/sunset.gif", 0.0D);
        this.clds = new Bckgrnd("/Background/clouds.gif", 0.1D);
        this.hll = new Bckgrnd("/Background/mountains.gif", 0.2D);
        this.tm = new TileMp(30);
        //this.tm.loadMap("Maps/level1c.map");
        this.tm.loadTiles("/Tilesets/ruinstileset.gif");
        //this.tm.loadTiles("/Tilesets/firetileset.gif");
        this.tm.loadMap("/Maps/level1a.map");
        ...
        try {
            this.txt = ImageIO.read(getClass().getResourceAsStream("/HUD/HageonTemple.gif"));
            //this.txt = ImageIO.read(new FileInputStream("HUD/HageonTemple.gif"));
    ...
    public void update() {
        ...
        this.tm.setPosition(160 - this.plr.getX(), 120 - this.plr.getY());
        //this.tm.setPosition(320/2 - this.plr.getX(), 240/2 - this.plr.getY());
    ...
    public void draw(Graphics2D grphcs) {
        this.sky.draw(grphcs);
        this.clds.draw(grphcs);
        this.hll.draw(grphcs);

        this.tm.draw(grphcs);
        for (int i = 0; i < this.enm.size(); i++) {
            ((Foe) this.enm.get(i)).draw(grphcs);
        }
        for (int i = 0; i < this.eprjctl.size(); i++) {
            ((Projectile) this.eprjctl.get(i)).draw(grphcs);
        }
        for (int i = 0; i < this.bm.size(); i++) {
            ((Booms) this.bm.get(i)).draw(grphcs);
        }
        this.plr.draw(grphcs);

        this.hb.draw(grphcs);

        this.hud.draw(grphcs);
        if (this.ttl != null) {
            this.ttl.draw(grphcs);
        }
        if (this.sbttl != null) {
            this.sbttl.draw(grphcs);
        }
        grphcs.setColor(Color.BLACK);
        for (int i = 0; i < this.tb.size(); i++) {
            grphcs.fill((Shape) this.tb.get(i));
        }
    }
